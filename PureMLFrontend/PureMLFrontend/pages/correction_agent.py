...
csv_data2 = pd.read_csv("pavel_agent_output.csv")


@template(route="/correction_agent", title="ColAgent")
def correction_agent() -> rx.Component:
    return rx.flex(
        rx.box(
                rx.heading(
                    "Data Consolidation - Determining equivalent but differing feature values and performing data consolidation", 
                    size="8",
                    margin_left="20px",
                    margin_right="20px",
                ),
        ...
        rx.heading(
            ("Consolidated Feature Value Dataset - ")
        ), 
        rx.box(
        rx.data_table(
            data=csv_data2,
