...
csv_data2 = pd.read_csv("column_agent_output.csv")


@template(route="/column_agent", title="ColAgent")
def column_agent() -> rx.Component:
    return rx.flex(
        rx.box(
                rx.heading(
                    "Data Enrichment - Entirely New Data Features from Agent Powered Decision Making", 
                    size="8",
                    margin_left="20px",
                    margin_right="20px",
                ),
        ...
        rx.button(
                "Next Step",
                on_click=rx.redirect("/correction_agent"),
...
# @template(route="/column_agent", title="ColumnAgent")
# def column_agent() -> rx.Component:
#     return rx.flex(
#         rx.box(
#                 rx.heading(
